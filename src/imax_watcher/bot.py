...
async def watch_menu(interaction:discord.Interaction):
    ...
    if not showings: return await interaction.followup.send("현재 연결된 좌석 피드에 상영정보가 없습니다. README의 CGV_FEED_URL 설정을 확인하세요.",ephemeral=True)
...
async def on_ready():
    if not getattr(bot,"_synced",False):
        ...
        bot._synced=True
    if CHANNEL_ID and not getattr(bot,"_watcher_started",False):
